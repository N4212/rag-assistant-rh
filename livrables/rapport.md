...
- **Réindexation manuelle.** La prise en compte d'une modification du corpus
  suppose de relancer l'indexation à la main. Tant que cette opération n'est pas
  effectuée, la base vectorielle conserve l'ancienne version du document et
  l'assistant répond à partir d'une information périmée — précisément le
  scénario de dérive silencieuse décrit en introduction. L'indexation est en
  outre intégrale : la collection est supprimée puis reconstruite en totalité,
  ce qui reste instantané sur dix documents mais deviendrait coûteux sur un
  corpus d'entreprise réel.
- **Péremption du jeu de référence.** Les réponses attendues sont figées dans le
  jeu de questions. Si une procédure est révisée, la réponse attendue devient
  fausse et la métrique signale une dégradation là où le système a correctement
  répercuté le changement. L'étalon de mesure vieillit donc au même rythme que
  le corpus qu'il sert à évaluer, ce qui constitue un coût de maintenance
  récurrent.
...
- **Automatiser la réindexation.** Faire surveiller le dossier du corpus par
  l'orchestrateur permettrait de déclencher une réindexation dès qu'un document
  est ajouté ou modifié, suivie immédiatement d'une évaluation. Le flux
  deviendrait : détection du changement, réindexation, mesure, alerte en cas de
  dégradation. Cette chaîne traiterait la principale source de dérive identifiée
  sans intervention humaine. Une indexation incrémentale, ne recalculant que les
  fragments des documents modifiés, s'imposerait par ailleurs à mesure que le
  corpus grossit.
- **Rattacher le jeu de référence au corpus.** Associer chaque question au
  document et à la version dont elle dépend permettrait de signaler
  automatiquement les questions à réviser lorsque le document source change,
  plutôt que de découvrir la péremption par une fausse alerte.
